...
```{R}
df1$Sex <- as.factor(ifelse(df1$Sex == "male", 0, 1))
df1$y <- factor(df1$y)
levels(df1$y) <- make.names(levels(df1$y))
...
lasso_fit <- train(
  x = df1 %>% select(Pclass, Sex, Age, Fare, siblings_spouses, parents_children) %>% as.data.frame(),
  y = df1$y,
  method = 'glmnet',
  trControl = controls, 
